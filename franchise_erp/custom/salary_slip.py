...
def apply_leave_rule_deductions(doc, method):
    ...
    if unit:
        ...
        for sl in short_leaves:
            # No sandwich rule for short leave: exclude holidays that fall
            # within the range instead of counting every calendar day.
            holiday_dates = get_holiday_dates_for_employee(doc.employee, sl.from_date, sl.to_date)
            total_calendar_days = date_diff(sl.to_date, sl.from_date) + 1
            working_days = total_calendar_days - len(holiday_dates)

            if working_days < 0:
                working_days = 0

            total_days += unit * working_days
