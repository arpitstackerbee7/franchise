...
function open_purchase_order_mapper(frm) {
    if (!frm.doc.consignor) frappe.throw("Please select Supplier first");

    frappe.call({
        method: "franchise_erp.franchise_erp.doctype.incoming_logistics.incoming_logistics.get_used_source_ids",
        args: { source_doctype: "Purchase Order" },
        callback: function(r) {
            const used_ids = r.message || [];
            new frappe.ui.form.MultiSelectDialog({
                doctype: "Purchase Order",
                target: frm,
                setters: { supplier: frm.doc.consignor, company: frm.doc.owner_site },
                get_query() {
                    return {
                        filters: [
                            ...
                            ["Purchase Order", "name", "not in", used_ids] // 🚩 Filter Added
                        ]
                    };
                },
                action(selections) {
...
function open_job_receipt_mapper(frm) {
    frappe.call({
        method: "franchise_erp.franchise_erp.doctype.incoming_logistics.incoming_logistics.get_used_source_ids",
        args: { source_doctype: "Subcontracting Order" },
        callback: function(r) {
            const used_ids = r.message || [];
            new frappe.ui.form.MultiSelectDialog({
                doctype: "Subcontracting Order",
                target: frm,
                setters: { company: frm.doc.owner_site },
                get_query() {
                    return {
                        filters: [
                            ["Subcontracting Order", "docstatus", "=", 1],
                            ["Subcontracting Order", "company", "=", frm.doc.owner_site],
                            ["Subcontracting Order", "name", "not in", used_ids] // 🚩 Filter Added
                        ]
                    };
                },
                action(selections) {
...
function open_sales_return_mapper(frm) {
    if (!frm.doc.consignor_customer) frappe.throw("Please select Customer first");

    
    frappe.call({
        method: "franchise_erp.franchise_erp.doctype.incoming_logistics.incoming_logistics.get_used_source_ids",
        args: { source_doctype: "Sales Invoice" }, // Ensure name is "Sales Invoice"
        callback: function(r) {
            const used_ids = r.message || [];
            
            // Add this line for debugging to check whether the list is being received or not.
            // console.log("Used Sales Invoices:", used_ids);
                ...
                get_query() {
                    return {
                        filters: [
                            ...
                            ["Sales Invoice", "customer", "=", frm.doc.consignor_customer],
                            ["Sales Invoice", "company", "=", frm.doc.owner_site],
                            ["Sales Invoice", "name", "not in", used_ids] // 🚩 YAHI FILTER HAI
                        ]
                    };
                },
                action(selections) {
