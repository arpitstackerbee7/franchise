...
def get_company_columns(companies, filters):
    ...
    # Counter ka represented company sirf ek baar nikalo
    counter_company = filters.get("counter_company")
    ...
    for company in companies:
        apply_currency_formatter = 0 if filters.get("presentation_currency") else 1

        currency = (
            filters.get("presentation_currency")
            or erpnext.get_company_currency(company)
        )

        # Default label Company Name
        label = company

        # Counter Company ke column ka label Customer Name kar do
        if counter_company and company == counter_company:
            label = filters.get("counter")
...
def get_data(companies, root_type, balance_must_be, fiscal_year, filters=None, ignore_closing_entries=False):
	accounts, accounts_by_name, parent_children_map = get_account_heads(
		root_type, companies, filters
	)
	...
	for entries in gl_entries_by_account.values():
		for entry in entries:
			...
			if entry.account_number:
				account_name = f"{entry.account_number} - {entry.account_name}"
			else:
			...
			for company in companies.keys():
				...
				if (
					...
					d[company] = d.get(company, 0) + debit - credit

					if entry.posting_date < getdate(start_date):
						d["company_wise_opening_bal"][company] += debit - credit

			if entry.posting_date < getdate(start_date):
				d["opening_balance"] = d.get("opening_balance", 0) + debit - credit
...
def get_companies(filters, counter_company=None):
    companies = {}

    # Main Company
    main_company = filters.get("company")

    # Counter Company selected
    if counter_company:
        company_list = [main_company, counter_company]

        companies[main_company] = [main_company]
        companies[counter_company] = [counter_company]

        return company_list, companies

    # Original Behaviour
    company_list = get_subsidiary_companies(main_company)

    if main_company not in company_list:
        company_list.insert(0, main_company)

    companies[main_company] = company_list

    for company in company_list:
        if company not in companies:
            companies[company] = get_subsidiary_companies(company)
...
def prepare_data(accounts, start_date, end_date, balance_must_be, companies, company_currency, filters):
	...
	for d in accounts:
		...
		for company in companies.keys():
			...
			if abs(value) >= get_zero_cutoff(filters.get("presentation_currency")):
				has_value = True
				total += value
...
def set_gl_entries_by_account(
	...
	# Main + Counter Company
	selected_companies = [filters.get("company")]

	if filters.get("counter_company"):
		selected_companies.append(filters.get("counter_company"))

	companies = frappe.get_all(
		"Company",
		filters={"name": ["in", selected_companies]},
