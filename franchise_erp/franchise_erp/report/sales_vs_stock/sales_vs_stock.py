...
def get_chart_data(data):
    return {
        "data": {
            "labels": [d.get("custom_barcode_code") or "No Style" for d in data],
            "datasets": [
                {"name": "Sales", "values": [float(d.get("sales_qty") or 0) for d in data]},
                {"name": "Stock", "values": [float(d.get("stock_qty") or 0) for d in data]},
            ],
        },
        "type": "bar",
    }